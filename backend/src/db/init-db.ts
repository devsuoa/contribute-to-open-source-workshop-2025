...
const seedDb = () => {
  ...
  const PROBLEMS = [
    ...
    { title: 'Sum of Evens', description: 'What is the sum of all even numbers from 1 to 100?', solution: '2550' },
  ];

  const COMPETITIONS = [
    { name: 'Weekly Challenge', start_time: new Date(Date.now() + 24 * 60 * 60 * 1000), end_time: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) },
    { name: 'Past Challenge', start_time: new Date(Date.now() - 14 * 24 * 60 * 60 * 1000), end_time: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) }
  ]
  ...
  db.serialize(() => {
    ...
    const insertCompProblem = db.prepare('INSERT INTO competition_problems (competition_id, problem_id) VALUES (?, ?)');
