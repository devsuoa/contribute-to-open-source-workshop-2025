...
export const getProblemsByCompetitionId = (
...
export const getPastCompetitions = (): Promise<Competition[]> => {
  return new Promise<Competition[]>(async (resolve, reject) => {
    const now = new Date();
    db.all(
      "SELECT * FROM competitions WHERE end_time < ?",
      [now],
      async (err, rows: any[]) => {
        if (err) {
          return reject(err);
...
export const getUserProblemStatus = (
  userId: number,
  problemId: number,
): Promise<UserProblemStatus | null> => {
  return new Promise<UserProblemStatus | null>((resolve, reject) => {
    db.get(
      "SELECT * FROM user_problem_status WHERE user_id = ? AND problem_id = ?",
      [userId, problemId],
      (err, row: any) => {
        if (err) {
          return reject(err);
        ...
        resolve(
          row
            ? {
                ...
                last_attempt: row.last_attempt && new Date(row.last_attempt),
                solved: !!row.solved,
              }
            : null,
        );
...
export const createUserProblemStatus = (
  userId: number,
  problemId: number,
): Promise<UserProblemStatus> => {
  return new Promise<UserProblemStatus>((resolve, reject) => {
    db.run(
      "INSERT INTO user_problem_status (user_id, problem_id, last_attempt, solved) VALUES (?, ?, ?, ?)",
...
export const updateUserProblemStatus = (
  ...
  return new Promise<void>((resolve, reject) => {
    db.run(
      "REPLACE INTO user_problem_status (user_id, problem_id, last_attempt, solved) VALUES (?, ?, ?, ?)",
      [
        userProblemStatus.user_id,
        userProblemStatus.problem_id,
        userProblemStatus.last_attempt && userProblemStatus.last_attempt,
        userProblemStatus.solved ? 1 : 0,
      ],
      function (err) {
        if (err) {
          return reject(err);
...
export const getUserCompetitionStatus = (
  userId: number,
  competitionId: number,
): Promise<CompetitionUserStatus | null> => {
  return new Promise<CompetitionUserStatus | null>((resolve, reject) => {
    db.get(
      "SELECT * FROM competition_user_status WHERE user_id = ? AND competition_id = ?",
...
export const createUserCompetitionStatus = (
  userId: number,
  competitionId: number,
): Promise<CompetitionUserStatus> => {
  return new Promise<CompetitionUserStatus>((resolve, reject) => {
    db.run(
      "INSERT INTO competition_user_status (user_id, competition_id, points) VALUES (?, ?, ?)",
...
export const updateUserCompetitionStatus = (
  competitionUserStatus: CompetitionUserStatus,
): Promise<void> => {
  return new Promise<void>((resolve, reject) => {
    db.run(
      "REPLACE INTO competition_user_status (user_id, competition_id, points) VALUES (?, ?, ?)",
      [
        competitionUserStatus.user_id,
        competitionUserStatus.competition_id,
        competitionUserStatus.points,
      ],
      function (err) {
        if (err) {
          return reject(err);
...
export const createSubmission = (submission: Submission): Promise<void> => {
  return new Promise<void>((resolve, reject) => {
    db.run(
      "INSERT INTO submissions (competition_id, problem_id, user_id, content, submitted_at, verdict) VALUES (?, ?, ?, ?, ?, ?)",
      [
...
export const getSubmissions = (
  ...
  return new Promise<Submission[]>((resolve, reject) => {
    db.all(
      "SELECT * FROM submissions WHERE competition_id = ? AND problem_id = ? AND user_id = ? ORDER BY submitted_at DESC LIMIT 5",
